...
struct IAP: ViewModifier {
    private static let inAppPurchaseGroupId = "20516661"

    private enum Status: String {
        case loading, active, inactive
    }

    @Binding var premiumStatus: PremiumStatus
    ...
    func body(content: Content) -> some View {
        content
            ...
            .subscriptionStatusTask(for: Self.inAppPurchaseGroupId, priority: .background) { (state: EntitlementTaskState<[Product.SubscriptionInfo.Status]>) in
    ...
    private func updatePremiumStatus() {
        ...
        premiumStatus = hasPremium ? .active : .inactive
    }
