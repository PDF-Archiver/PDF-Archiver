...
| `focusedField` order: date → specification → tags → save | Confirm the VoiceOver swipe/rotor order (iOS) matches this sequence with nothing skipped or duplicated. On macOS, Tab is owned by `TabCycleModifier` (#320) and **wraps** save → date on purpose — that is correct behaviour, not a defect. **Do not change the order in code**; a genuine failure is a follow-up issue | sequential, wrapping on macOS |
| macOS keyboard reach (#320 interaction) | #320 marks the tag chips, the suggested-date buttons and "Today" `.focusable(false)` so Tab stays on the four form fields. Confirm VoiceOver (VO+arrows) and Voice Control still reach and activate them — both read the accessibility tree, not the keyboard focus ring, so they should. If they do not, `voice-control` must stay undeclared for MAC | reachable by VoiceOver and Voice Control, not by Tab |
