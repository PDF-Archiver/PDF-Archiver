...
- Foreground: `premiumStatus` is `@Shared(.premiumStatus)`, starts as `.loading` and is set by
  `AppFeature` from `PremiumDependency.currentStatus` at launch, after every `Transaction.updates`
  delivery and whenever the scene becomes active. The search reducer therefore observes it
...
- Cold background launch: there is no UI, so the in-memory status is empty. The background handler
  calls the same `PremiumDependency.currentStatus` before calling `indexPendingTexts`.
